/***************************************************
  NOTE:  This is modified from the original to attempt and get data from data.sparkfun.com
...
//#define WLAN_SSID		"FVYC9"
//#define WLAN_PASS       "D2Z98PBNS2YNFK9Z"

#define WLAN_SSID		"Galaxy S6 active 3713"
#define WLAN_PASS		"Brittany"
...
void setup(void)
{
	...
	{
		if(cc3000.getHostByName("data.sparkfun.com", &ip)) break;
		delay(1000);
	}
	if(0L == ip)
	{
	...
	Serial.print(F("Connecting to Sparkfun server..."));
	client = cc3000.connectTCP(ip, 80);
	if(client.connected())
	{
		Serial.print(F("connected.\r\nRequesting data..."));
		client.print(F("GET /json/ HTTP/1.1\r\nHost: data.sparkfun.com/input/OG55WRJE82cXlG34rzxa\r\n\r\n"));
	}
	else
	{
		Serial.println(F("failed"));
	...
	Serial.print("\r\nReading response...");
	Serial.flush();
	humidity[0] = temp[0] = timestamp[0] = 0; // Clear data
	jsonParse(0, 0);
	client.close();
	Serial.println(F("OK"));
	...
	for(;;)
	{
		...
		else if((c == '"') || (c == '\''))     // String follows
		{
			...
			else     // Value-reading mode
			{
				...
				if       (!strcasecmp(humidity, "humidity"))
				{
					strncpy(humidity, value, sizeof(humidity) - 1);
					Serial.println("Humidity Found");
				}
				else if(!strcasecmp(temp, "temp"))
				{
