...
POTENTIOMETER
  ...
  control the brightness of an LED. Turn the knob to change
  the LEDs intesity level
...
{
  // We'll be using pin 9 to light a LED, so we must configure it
  // as an output.
 
  // Because we already created a variable called ledPin, and
  // set it equal to 9, we can use "ledPin" in place of "9".
...
void loop() // this function runs repeatedly after setup() finishes
{
  ...
  value = map(sensorValue, 0, 1024, 0, 255);   
